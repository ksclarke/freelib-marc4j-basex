...
try {
  ...
  let $result :=
    ...
    then
      ...
      let $record := fetch:xml('src/test/resources/collection.marc.xml')//marcxml:record
      let $result := marc:write($record//marcxml:record, file:temp-dir() || '/collection-' || local:get-timestamp() || '.mrc')
      return $result
    else false()
  return
    if ($result)
